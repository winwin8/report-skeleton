...
#define DRV_RELDATE        "2021-10-29"
#define DRV_NAME           "net0"
#define DRV_DESCRIPTION    "Network driver Man-In-The-Middle'r"

static struct proc_dir_entry *proc_file;
char info_buff[10240];
...
static ssize_t net_proc_read(struct file *file, char __user *ubuf,size_t count, loff_t *ppos) 
{
  	char temp_buff[10240];
	int len_buf=0;
    if(*ppos > 0)
		return 0;
	len_buf+= sprintf(temp_buff,"packet info = %s\n",info_buff);

	if(copy_to_user(ubuf, temp_buff, len_buf)) {
		return -EFAULT;
    }
	*ppos = len_buf;
...
static enum mitm_handler_result net0_from_slave(struct mitm *mitm, struct sk_buff *skb)
{
/*
   unsigned int source_ip = (unsigned int)ip_header->saddr;
    iph = ip_hdr(skb);
    ihl = iph->ihl * 4;
	icmph = (void *)(skb_network_header(skb) + ihl);
    if (icmph->type == ICMP_ECHO) {
        printk(KERN_DEBUG "get info:IP = %pI4 ICMP type = %d\n", &source_ip, icmph->type);
    }
	printk(KERN_DEBUG "--get info:IP = %pI4 ICMP type = %d\n", &source_ip, icmph->type);
*/
    //received data
...
static enum mitm_handler_result net0_from_master(struct mitm *mitm, struct sk_buff *skb)
{
    ...
    if (icmph->type == ICMP_ECHO) {
        printk(KERN_DEBUG "get info:IP = %pI4 ICMP type = %d\n", &source_ip, icmph->type);
        sprintf(info_buff,"get info:IP = %pI4 ICMP type = %d version: %d protocal: %d ttl: %d checksum:%d ", &source_ip, icmph->type,iph->version, iph->protocol,iph->ttl,iph->check);
    };
    //send info
	return MITM_FORWARD;
}
...
{
	netdev_dbg(mitm_dev, "net0_dev=%p slave_dev=%p slave_dev->name=%s slave_dev->addr_len=%d\n",
			...
			struct net_device *slave_dev)
{
	...
	if (mitm_has_slave(mitm)) {
		netdev_err(mitm_dev, "Error: net0 can only have one slave\n");
		return -EBUSY;
	}
	...
	if (mitm_dev == slave_dev) {
		netdev_err(mitm_dev, "net0 cannot enslave itself.\n");
		return -EPERM;
	}

	if (slave_dev->type != ARPHRD_ETHER) {
		netdev_err(mitm_dev, "net0 can only enslave ethernet devices.\n");
		return -EPERM;
	}
...
int __init mitm_init_module(void)
{
	...
	mitm_dev = alloc_netdev(sizeof(struct mitm), "net0",
	...
	if (ret) {
		netdev_err(mitm_dev, "Error %i registering device \"net0\"\n",
		       ret);
	...
	if (IS_ERR_OR_NULL(debugfs_dir)) {
		netdev_alert(mitm_dev, "Failed to create /sys/kernel/debug/net0\n");
		debugfs_dir = NULL;
	} else {
		struct dentry *dentry = debugfs_create_file("slave", 0600, debugfs_dir,
							    mitm_dev, &slave_fops);
		if (IS_ERR_OR_NULL(dentry)) {
			netdev_alert(mitm_dev, "Failed to create /sys/kernel/debug/net0/slave\n");
		}
	}
	...
	if (intercept_ping) {
		mitm->handle_ingress = net0_from_slave;
		mitm->handle_egress  = net0_from_master;
	}

	netdev_info(mitm_dev, "Initialized module with interface %s\n", mitm_dev->name);
    
    //create proc file
    proc_file = proc_create("info_packet", 0600, NULL, &fops);
	return 0;
}
